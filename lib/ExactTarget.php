...
class ExactTarget {
	...
	private function makeRequest(array $xml_payload) {
		...
		$res_arr = $this->xml2array($result);
		
		if($res_arr['exacttarget']['system']) {
			return $res_arr['exacttarget']['system'];
		}
		return $res_arr;
	}
	...
	private function arrayToXML(array $arr, $rootTagName = 'root') {
	    return $this->array_to_xml($arr, new SimpleXMLElement('<'.$rootTagName.'/>'))->asXML();
	}
